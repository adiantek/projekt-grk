...
void main() {
    ...
    vec4 objectColor = texture(texAlbedo, textureCoords);

    float computedLightIntensity = 1.0;

	if (objectColor.a < 0.75)
		discard;
	...
	vec3 color = PBR(
        normal, viewDirection, objectColor.xyz, vec3(0.0),
        0.0, 0.0, texture(aoMap, textureCoords).r, lightDirection,
