...
void main() {
    ...
    vec3 waterPosition = vec3(vertexPosition.xy, vertexPosition.z + depthInfo.b + 5.0);//vertexPosition + depthInfo.xyz;
    vec3 waterNormal = normalize(normalInfo.xyz);

    oldPosition = waterPosition;


    vec4 projectedWaterPosition = transformation * vec4(waterPosition, 1.0);
    ...
    vec4 environment = texture(environmentMap, coords);
